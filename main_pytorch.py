"""
@File     : main_pytorch.py
@Time     : 2023/9/24 21:01
@Author   : 张伟杰
@Func     : 使用pytorch模型检测幼虫
"""
import os
...
if __name__ == "__main__":
    ...
    for image_name in images:
        ...
        h, w, c = image.shape
        my_find = FindContour(
            image, 2, True, False, doji_len=int((((h / 1080) + (w / 1920)) / 2) * 30)
        )
        ...
        for box_list in boxes_list:
            for index, box in enumerate(box_list):
                ...
                if SHOW_CUT_IMAGE:
                    cv2.imwrite(
                        f'{OUTPUTS_ROOT}/{SOURCE.split(".")[1]}_cut_{index}.jpg',
                        cut_image,
                    )
                try:
                    ...
                    continue
            cv2.imwrite(f'{OUTPUTS_ROOT}/{image_name.split(".")[0]}_detect.jpg', image)
